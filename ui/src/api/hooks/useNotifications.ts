...
export function useDeleteNotification() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => apiRequest<void>(`/notifications/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['notifications'] }),
  });
}
...
export interface NotificationChannelDetail extends Omit<NotificationChannel, 'config'> {
  config: Record<string, unknown>;
}
...
export function useNotificationChannel(id: string | null) {
  return useQuery({
    ...
    queryFn: () => apiRequest<NotificationChannelDetail>(`/notifications/${id}`),
    enabled: !!id,
  });
}
