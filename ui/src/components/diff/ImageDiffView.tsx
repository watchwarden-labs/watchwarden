...
export function ImageDiffView({ diff, compact }: ImageDiffViewProps) {
  if (diff.changeCount === 0) {
    return <p className="text-sm text-muted-foreground">No configuration changes detected.</p>;
  }
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      {diff.envAdded?.length || diff.envRemoved?.length || diff.envChanged?.length ? (
        <DiffSection title="Environment Variables">
          ...
          {diff.envChanged?.map((e) => (
            <DiffLine key={e.key} type="changed" text={`${e.key}: ${e.oldValue} → ${e.newValue}`} />
          ))}
        </DiffSection>
      ) : null}
      ...
      {diff.entrypointChanged && (
        <DiffSection title="Entrypoint">
          <DiffLine type="removed" text={diff.entrypointChanged.old?.join(' ') || '(none)'} />
          <DiffLine type="added" text={diff.entrypointChanged.new?.join(' ') || '(none)'} />
        </DiffSection>
      )}
      ...
      {diff.cmdChanged && (
        <DiffSection title="Command">
          <DiffLine type="removed" text={diff.cmdChanged.old?.join(' ') || '(none)'} />
          <DiffLine type="added" text={diff.cmdChanged.new?.join(' ') || '(none)'} />
        </DiffSection>
      )}
      ...
      diff.labelsChanged?.length ? (
        <DiffSection title="Labels">
          ...
          {diff.labelsChanged?.map((l) => (
            <DiffLine key={l.key} type="changed" text={`${l.key}: ${l.oldValue} → ${l.newValue}`} />
          ))}
        </DiffSection>
      ) : null}
...
function DiffLine({ type, text }: { type: 'added' | 'removed' | 'changed'; text: string }) {
  const icon =
    type === 'added' ? (
      <Plus size={12} />
    ) : type === 'removed' ? (
  ...
  const color =
    type === 'added' ? 'text-success' : type === 'removed' ? 'text-destructive' : 'text-warning';
  const bg =
    type === 'added' ? 'bg-success/5' : type === 'removed' ? 'bg-destructive/5' : 'bg-warning/5';

  return (
    <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded ${bg} ${color}`}>
      {icon}
