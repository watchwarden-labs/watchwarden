...
export function VersionPickerModal({
  ...
  const { data: versions, isLoading } = useVersions(agentId, container.docker_id, open);
  ...
  const registryInfo = needsSearchQuery ? searchResult?.registry : versions?.registry;
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <br />
              Current: <code className="text-xs">{truncateDigest(container.current_digest)}</code>
            </DialogDescription>
          </DialogHeader>
          ...
          <Tabs defaultValue="registry" className="mt-2">
            ...
            {/* Local History Tab */}
            <TabsContent value="local" className="mt-3 max-h-80 overflow-y-auto">
              {isLoading && (
              ...
              {!isLoading && (!versions?.local || versions.local.length === 0) && (
            ...
            {/* Changes Tab */}
            {container.last_diff && (
              <TabsContent value="changes" className="mt-3 max-h-80 overflow-y-auto">
                {(() => {
                  try {
                    ...
                    return (
                      <p className="text-sm text-muted-foreground">Unable to parse diff data.</p>
                    );
                  }
                })()}
              </TabsContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {selected?.label ?? 'the selected version'}. The container will be briefly unavailable
              during the process.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
