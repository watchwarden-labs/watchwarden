...
export function AgentDetail() {
	...
	const handleDelete = () => {
		if (
			confirm(`Remove agent "${agent.name}"? This will delete all history.`)
		) {
											...
											{ id: agent.id, keepPrevious: pruneKeep, dryRun: true },
											{
												onSuccess: () =>
													addToast({
														type: "info",
														message:
															"Dry-run prune initiated — check agent logs for details",
													}),
											},
										)
									}
														...
														{ id: agent.id, keepPrevious: pruneKeep },
														{
															onSuccess: () =>
																addToast({
																	type: "success",
																	message:
																		"Prune initiated — old images will be removed",
																}),
															onError: () =>
																addToast({
																	type: "error",
				...
				<div className="space-y-1">
					...
					<div className="flex gap-2">
						<Button
							size="sm"
							variant={
								policy.strategy === "stop-first" || !policy.strategy
									? "default"
									: "outline"
							}
							onClick={() =>
						...
						<Button
							size="sm"
							variant={
								policy.strategy === "start-first" ? "default" : "outline"
							}
							onClick={() =>
								updatePolicy.mutate({
									scope: `agent:${agentId}`,
					...
					<p className="text-xs text-muted-foreground">
						Stop-first: stop old → start new. Blue-green: start new → verify
						health → stop old.
					</p>
				</div>
				<p className="text-xs text-muted-foreground">
					After each update, the container is monitored for the stability
					window. If unhealthy for longer than the max unhealthy threshold, it
