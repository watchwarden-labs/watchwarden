...
export function Agents() {
  ...
  const handleDelete = (e: React.MouseEvent, agentId: string, agentName: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (confirm(`Remove agent "${agentName}"? This will delete all history for this agent.`)) {
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-muted-foreground text-lg">
              {filter === 'All' ? 'No agents connected' : `No ${filter.toLowerCase()} agents`}
            </p>
            {filter === 'All' && (
              <>
                <p className="text-muted-foreground text-sm">Register a new agent to get started</p>
                <Button onClick={() => setRegisterOpen(true)}>Add your first agent</Button>
              </>
            )}
          </CardContent>
        </Card>
