...
} from "../notifications/session-batcher.js";
...
export class AgentHub {
	...
	handleConnection(socket: WebSocket): void {
		...
		socket.on("error", (err) => {
			log.warn(
				"hub",
				`WS error for agent ${agentId ?? "unknown"}: ${err.message}`,
			);
		});
					...
					switch (message.type) {
						...
						case "CHECK_RESULT": {
							...
							for (const r of payload.results) {
								...
								await updateContainerDiff(
									r.containerId,
									r.diff ? JSON.stringify(r.diff) : null,
								);
							}
							await updateAgentStatus(agentId, "online", Date.now());
							const updatesAvailable = payload.results.filter(
								(r) => r.hasUpdate,
							).length;
							this.broadcaster?.broadcast({
								type: "CHECK_COMPLETE",
							...
							const globalAutoUpdate =
								(await getConfig("auto_update_global")) === "true";
							const shouldAutoUpdate =
								(agentRecord?.auto_update === 1 || globalAutoUpdate) &&
								updatesAvailable > 0;
							...
							if (shouldAutoUpdate) {
								...
								const withUpdates = payload.results
									...
									.map((r) => {
										...
										const containerName =
											dbContainer?.name ?? r.containerName ?? r.containerId;
								...
								if (withUpdates.length > 0) {
									const agent = (await listAgents()).find(
										(a) => a.id === agentId,
									);
									dispatchCheckResults(
										agent?.name ?? agentId,
						...
						case "UPDATE_RESULT": {
							...
							const autoRolledBack = (payload as Record<string, unknown>)
								.autoRolledBack as boolean | undefined;
							const rollbackReason = (payload as Record<string, unknown>)
								.rollbackReason as string | undefined;
							...
							if (autoRolledBack) {
								try {
									...
									const containerNames = results
										.map((r) => r.containerName)
										.filter(Boolean);
									const reason =
										rollbackReason || "Container unhealthy after update";
									...
									log.error(
										"hub",
										`Failed to dispatch auto-rollback notification: ${err}`,
									);
								}
							}
	 ...
	 */
	private writeAuditLogWithRetry(
		entry: Parameters<typeof insertAuditLog>[0],
	): void {
