...
describe("/ws/ui authentication", () => {
	...
	beforeAll(async () => {
		...
		app.get("/ws/ui", { websocket: true }, async (socket, request) => {
			...
			const queryToken = new URL(request.url, "http://x").searchParams.get(
				"token",
			);
			const token = cookieToken ?? queryToken;
			const secret = await setConfig("jwt_secret", JWT_SECRET).then(
				() => JWT_SECRET,
			);
			try {
	...
	it("closes with 4001 when ?token= is an invalid JWT", async () => {
		const ws = new WebSocket(
			`ws://127.0.0.1:${port}/ws/ui?token=not-a-valid-jwt`,
		);
		const { code } = await waitForClose(ws);
		expect(code).toBe(4001);
	});
