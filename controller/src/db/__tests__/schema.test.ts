...
describe('schema', () => {
  ...
  it('seeds default global_schedule config', async () => {
    const [row] = await sql`SELECT value FROM config WHERE key = 'global_schedule'`;
    expect(row).toBeDefined();
    expect(row?.value).toBe('0 4 * * *');
  });

  it('seeds default auto_update_global config', async () => {
    const [row] = await sql`SELECT value FROM config WHERE key = 'auto_update_global'`;
    expect(row).toBeDefined();
    expect(row?.value).toBe('false');
  });

  it('seeds default admin_password_hash config', async () => {
    const [row] = await sql`SELECT value FROM config WHERE key = 'admin_password_hash'`;
    expect(row).toBeDefined();
