...
export async function listAgentsByTokenPrefix(prefix: string): Promise<Agent[]> {
...
export async function getHistoryStats(): Promise<HistoryStats> {
  ...
  await sql.begin(async (txBase) => {
    ...
    const successRate =
      totalUpdates > 0 ? (Number(successRow?.count ?? 0) / totalUpdates) * 100 : 0;
...
export async function listRegistryCredentials(): Promise<RegistryCredential[]> {
  const rows = await sql`SELECT * FROM registry_credentials ORDER BY created_at DESC`;
...
export async function listNotificationChannels(): Promise<NotificationChannel[]> {
  const rows = await sql`SELECT * FROM notification_channels ORDER BY created_at DESC`;
...
export async function getNotificationLogs(limit = 50, offset = 0): Promise<NotificationLog[]> {
  const rows =
...
export async function getUpdatePolicy(scope: string): Promise<UpdatePolicy | undefined> {
...
export async function getEffectivePolicy(agentId?: string): Promise<UpdatePolicy> {
  if (agentId) {
...
function mapRegistryCredential(row: Record<string, unknown>): RegistryCredential {
  return {
    id: row.id as string,
...
function mapNotificationChannel(row: Record<string, unknown>): NotificationChannel {
  return {
    id: row.id as string,
