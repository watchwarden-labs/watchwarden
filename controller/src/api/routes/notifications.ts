...
const notificationRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { id: string } }>('/api/notifications/:id', async (request, reply) => {
    const ch = await getNotificationChannel(request.params.id);
    if (!ch) return reply.code(404).send({ error: 'Channel not found' });
    try {
      const decryptedConfig = JSON.parse(decrypt(ch.config)) as Record<string, unknown>;
      // Mask sensitive values — show only last 4 chars
      const masked: Record<string, unknown> = {};
      const sensitiveKeys = new Set(['botToken', 'webhookUrl', 'password', 'apiKey', 'secret']);
      ...
      request.log.error(err, `Failed to decrypt notification channel ${ch.id}`);
      return { ...ch, config: {} };
    }
  });
  ...
  fastify.post<{
    ...
    const { type, name, config, events, enabled, template, link_template } = request.body;
    if (!type || !name || !config || !events) {
      return reply.code(400).send({ error: 'type, name, config, and events are required' });
    }
    const id = uuidv4();
  ...
  fastify.put<{
    ...
    const { name, config, events, enabled, template, link_template } = request.body;
    await updateNotificationChannel(request.params.id, {
      ...(name !== undefined ? { name } : {}),
      ...(config !== undefined ? { config: encrypt(JSON.stringify(config)) } : {}),
