...
import { interpolateTemplate, parseImageComponents, renderImageLink } from '../template-helpers.js';
...
describe('interpolateTemplate', () => {
  ...
  it('leaves unknown variables empty', () => {
    expect(interpolateTemplate('{{known}} {{unknown}}', { known: 'yes' })).toBe('yes ');
  });
});
...
describe('renderImageLink', () => {
  ...
  it('uses custom template', () => {
    const link = renderImageLink('myuser/myapp:v1', 'https://example.com/{{repository}}/{{tag}}');
