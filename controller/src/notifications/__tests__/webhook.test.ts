...
describe('webhook SSRF protection (SEC-02)', () => {
  ...
  it('blocks webhook to localhost', async () => {
    await expect(sendWebhook({ url: 'http://localhost:8080/hook' }, testEvent)).rejects.toThrow(
      /private\/internal/,
    );
  });

  it('blocks webhook to 127.0.0.1', async () => {
    await expect(sendWebhook({ url: 'http://127.0.0.1:8080/hook' }, testEvent)).rejects.toThrow(
      /private\/internal/,
    );
  });

  it('blocks webhook to 10.x.x.x private IP', async () => {
    await expect(sendWebhook({ url: 'http://10.0.0.1/hook' }, testEvent)).rejects.toThrow(
      /private\/internal/,
    );
  });
  ...
  it('blocks webhook to 169.254.x.x link-local', async () => {
    await expect(
      sendWebhook({ url: 'http://169.254.169.254/latest/meta-data' }, testEvent),
  ...
  it('blocks domain resolving to private IP (DNS rebinding)', async () => {
    ...
    await expect(sendWebhook({ url: 'https://evil.example.com/hook' }, testEvent)).rejects.toThrow(
      /private\/internal IP/,
    );
  });
