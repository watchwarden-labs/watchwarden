...
export function addUpdateResult(
	agentId: string,
	...
	const event: NotificationEvent = result.success
		...
		: {
				...
				agentName: agentId,
				containers: [
					{ name: result.containerName, error: result.error ?? "unknown" },
				],
			};
	notifier
		.dispatch(event)
...
function flushCheckBatch(): void {
	...
	const agents = pendingCheckBatch.agents.filter(
		(a) => a.containers.length > 0,
	);
	...
	if (containers.length === 0) {
		...
		if (pendingCheckBatch && agentId) {
			...
			if (
				...
				pendingCheckBatch.receivedAgentIds.size >=
					pendingCheckBatch.expectedAgentCount
			) {
				clearTimeout(pendingCheckBatch.timer);
				flushCheckBatch();
			}
