...
async function start() {
  ...
  if (!adminHash) {
    ...
    if (!password) throw new Error('ADMIN_PASSWORD env var is required on first startup');
  ...
  if (!jwtSecret) {
    ...
    if (!secret) throw new Error('JWT_SECRET env var is required on first startup');
  ...
  app.get('/ws/ui', { websocket: true }, async (socket, request) => {
    ...
    const token = (request.cookies as Record<string, string | undefined>)?.ww_token;
